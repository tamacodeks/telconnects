...
@section('content')
                        ...
                        <div class="row m-b-20" id="spinnerRow">
                            ...
                            <div class="col-md-8 m-b-20">
                                <form id="frmDingTopup" class="form-horizontal">
                                    ...
                                    <input type="hidden" name="display_text" id="display_text">
                                    <input type="hidden" name="description" id="description">
                                    <input type="hidden" name="ReceiveValue" id="ReceiveValue">
                                    <input type="hidden" name="SendValue" id="SendValue">
                                    <input type="hidden" name="sendCurrencyIso" id="sendCurrencyIso">
                                    <input type="hidden" name="receiveCurrencyIso" id="receiveCurrencyIso">
    ...
    <script src="{{ secure_asset('js/jquery.storageapi.min.js') }}" type="text/javascript"></script>
    <script>
    ...
    <script>
        $(document).ready(function () {
            storage.set('current_range_amount', '0');
            setTimeout(function () {
                // $("#mobile").focus();
                $("#frmDingTopup").submit();
            }, 1500)
            var validator = $("#frmDingTopup").validate({
                ...
                unhighlight: function (element) {
                    ...
                    if(storage.get('current_range_amount') != 0 && storage.get('current_range_amount') == $("#range_amount").val()){
            ...
            $('#range_amount').keypress(function () {
                var val = $(this).val();
                if (isNaN(val)) {
            ...
            $("#frmDingTopup").submit(function (e) {
                ...
                gettransferservice(api_base_url + "/tama-topup/fetchtransfer/providers?accountNumber=" + mobileNumber + "&countryCode=" + countryCode + "&countryIsos=" + countryIso);
            });
            ...
            $('#frmDingTopup input').keydown(function (e) {
                if (e.keyCode == 13) {
            ...
            $("#range_amount").blur(function () {
                ...
                if ($(this).val() != "" && currentVal >= min && currentVal <= max) {
                    ...
                    var actual_amount = (range_amount);
                    var local = (actual_amount * exchange_rate);
            ...
            $("#reviewOrderBtn").click(function () {
                var name, mobile ,skuCode, validity, display_text, description, ReceiveValue, SendValue,
                    sendCurrencyIso, query, receiveCurrencyIso,countryCode,countryIso, operator_id,operator_name,country;
                name = $("#operator_name").val()+' '+$("#local_amount1").val()+' '+$("#receiveCurrencyIso").val();
                mobile = $("#mobile").val();
                skuCode = $("#operator_id").val();
                validity = $("#validity").val();
                display_text = $("#local_amount1").val();
                description = $("#description").val();
                ReceiveValue = $("#SendValue").val();
                SendValue = $("#SendValue").val();
                sendCurrencyIso = $("#sendCurrencyIso").val();
                receiveCurrencyIso = $("#receiveCurrencyIso").val();
                countryCode = $("#countryCode").val();
                countryIso = $("#countryIso").val();
                operator_id = $("#skuCode").val();
                operator_name = $("#operator_name").val();
                country = $("#country").val();

                query = "name=" + name + "&mobile=" + mobile + "&skuCode=" + skuCode + "&validity=" + validity + "&display_text=" + display_text + "&description=" + description + "&ReceiveValue=" + ReceiveValue + "&SendValue=" + SendValue + "&sendCurrencyIso=" + sendCurrencyIso + "&receiveCurrencyIso=" + receiveCurrencyIso + "&countryCode=" + countryCode + "&countryIso=" + countryIso + "&operator_id=" + operator_id + "&operator_name=" + operator_name + "&country=" + country;
                console.log(query);
                if (name != "" && mobile != "" && skuCode != ""  && ReceiveValue != "" && SendValue != "" && countryCode != "" && countryIso != "") {
                    AppModal("{{ secure_url('tama-topup/transfer-review?') }}" + query, "{{ trans('tamatopup.order_summary') }}");
                } else {
                    $.alert({
