...
@section('content')
    ...
    <div class="container-fluid">
     ...
     margin-top: 5px;font-size: .9em;">
                                                        ...
                                                        @if(!empty($card->validity) || isset($aleda_service))
                                                            <tr>
...
//                       frameDoc.document.write('<link href="style.css" rel="stylesheet" type="text/css" />');
        ...
        $(document).ready(function () {
            ...
            $("#printMe").click(function () {
                if (isFirefox) {
                    var k = localStorage.getItem('print_card');
                    if (k === null) {
                        k = 0;
                    }
                    k++;
                    localStorage.setItem("print_card", k);
                    if (k > 1) {
                        ...
                        var serializedData = {
                            cc_id: "{{ $card->cc_id }}",
                            ccp_id: "{{ $card->ccp_id }}"@if(isset($aleda_service)),
                            aleda_service: "{{ $aleda_service }}" @elseif(isset($bimedia_service)),
                            bimedia_service: "{{ $bimedia_service }}" @endif };

                        // Fire off the request to /form.php
                        request = $.ajax({
                            url: "{{ secure_url(isset($bimedia_service) ? "print_callingcard" : (isset($aleda_service) ? "calling-cards/print/aleda" : "print_card_activated")) }}",
                            type: "POST",
                            headers: {
