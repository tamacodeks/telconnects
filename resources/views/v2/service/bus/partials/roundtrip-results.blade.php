@php
    ...
    $formatTrip = function ($trip) use ($normalizeAmenities) {
        if (is_array($trip)) {
            $trip['amenities'] = $normalizeAmenities($trip['amenities'] ?? []);
        }
        return base64_encode(json_encode($trip));
    };
...
<section class="bus-v2-panel bus-v2-results-panel bus-v2-roundtrip-results bus-v2-rt-split bus-v2-rt-stepflow is-choosing-departure" data-bus-v2-roundtrip-flow>

    {{-- ── Header ────────────────────────────────────────────────────────── --}}
    <div class="bus-v2-rt-split-header bus-v2-results-hero--reference">
        <div class="bus-v2-rt-split-header-main">
            <div class="bus-v2-results-step-row">
                <span class="bus-v2-results-step-pill">Step 1 of 2</span>
        ...
        <div class="bus-v2-results-meta-row">
            <span class="bus-v2-results-meta-chip"><i class="fas fa-route"></i> {{ $searchData['from_name'] ?? '' }} ⇄ {{ $searchData['to_name'] ?? '' }}</span>
    ...
    {{-- ── Selection recap bar (shown once outbound is picked) ───────────── --}}
    <div class="bus-v2-rt-selection-bar" data-rt-selection-bar hidden>
        ...
        <div class="bus-v2-rt-selection-leg bus-v2-rt-selection-leg--pending" data-rt-sel-return>
            ...
            <strong>Choose return trip →</strong>
        </div>
    ...
    <div class="bus-v2-rt-split-body">
        ...
        {{-- LEFT COLUMN: Outbound trips ──────────────────────────────────── --}}
        <div class="bus-v2-rt-split-col bus-v2-rt-split-col--outbound">
            <div class="bus-v2-rt-split-col-head">
                <div class="bus-v2-rt-split-col-title">
                    <span class="bus-v2-rt-split-col-icon bus-v2-rt-split-col-icon--out"><i class="fas fa-arrow-right"></i></span>
                    <div>
                        <h3>Departure</h3>
                        <small>{{ $searchData['from_name'] ?? '' }} → {{ $searchData['to_name'] ?? '' }} · {{ $searchData['departure'] ?? '' }}</small>
                    </div>
                </div>
                <span class="bus-v2-rt-split-count">{{ count($outboundResults) }} option{{ count($outboundResults) !== 1 ? 's' : '' }}</span>
            </div>

            <div class="bus-v2-result-list bus-v2-rt-split-list">
                @forelse ($outboundResults as $bus)
                    ...
                    <article class="bus-v2-trip-card {{ $m['isBla'] ? 'is-bla' : 'is-flix' }}{{ $selectable ? '' : ' is-disabled' }} bus-v2-rt-outbound-card"
                        ...
                        style="--bus-v2-stagger: {{ min($loop->index, 12) * 55 }}ms"
                    >
                        <div class="bus-v2-trip-card-top">
                            ...
                            <div class="bus-v2-trip-supporting">
                                <span class="bus-v2-trip-chip bus-v2-trip-chip--pax"><i class="fas fa-users"></i> {{ $m['pax'] }}</span>
                            </div>
                        </div>
                        ...
                        <div class="bus-v2-trip-card-body">
                            <div class="bus-v2-trip-main">
                                ...
                                <div class="bus-v2-route-operator-card {{ $m['isBla'] ? 'is-bla' : 'is-flix' }}">
                                    <span class="bus-v2-route-operator-logo">
                                        <img src="{{ $m['logo'] }}" alt="{{ $m['operator'] }}">
                                    </span>
                            ...
                            <div class="bus-v2-trip-side">
                                ...
                                <div class="bus-v2-trip-action bus-v2-rt-action-stack">
                                    <button type="button"
                                        class="bus-v2-button bus-v2-button--ghost bus-v2-rt-details-btn"
                                        data-rt-view-details
                                        data-rt-direction="outbound"
                                        data-rt-trip="{{ $formatTrip($bus) }}">
        ...
        <div class="bus-v2-rt-split-col bus-v2-rt-split-col--return">
            <div class="bus-v2-rt-split-col-head">
                <div class="bus-v2-rt-split-col-title">
                    <span class="bus-v2-rt-split-col-icon bus-v2-rt-split-col-icon--ret"><i class="fas fa-arrow-left"></i></span>
                    <div>
                        <h3>Return</h3>
                        <small>{{ $searchData['to_name'] ?? '' }} → {{ $searchData['from_name'] ?? '' }} · {{ $searchData['return_date'] ?? '' }}</small>
                    </div>
                </div>
                <span class="bus-v2-rt-split-count">{{ count($inboundResults) }} option{{ count($inboundResults) !== 1 ? 's' : '' }}</span>
            </div>

            {{-- Placeholder: shown until outbound is selected --}}
            <div class="bus-v2-rt-return-placeholder" data-rt-return-placeholder>
                <div class="bus-v2-rt-return-placeholder-inner">
                    ...
                    <p>Choose your outbound trip on the left to see available return journeys</p>
                </div>
            </div>

            {{-- Per-provider return forms (filtered by JS) --}}
                    ...
                    >
                        ...
                        <div class="bus-v2-result-list bus-v2-rt-split-list">
                            @forelse ($groupedInbound[$provKey] as $bus)
                                ...
                                <label class="bus-v2-trip-card bus-v2-rt-return-card {{ $m2['isBla'] ? 'is-bla' : 'is-flix' }}{{ $m2['available'] ? '' : ' is-disabled' }}"
                                    data-trip-value="{{ $formatTrip($bus) }}"
                                    style="--bus-v2-stagger: {{ min($loop->index, 12) * 55 }}ms">
                                    <input
                                        type="radio"
                                    ...
                                    <div class="bus-v2-trip-card-top">
                                        ...
                                        <div class="bus-v2-trip-supporting bus-v2-trip-supporting--return">
                                            <span class="bus-v2-trip-chip bus-v2-trip-chip--pax"><i class="fas fa-users"></i> {{ $m2['pax'] }}</span>
                                        </div>
                                        <div class="bus-v2-rt-select-indicator"><i class="fas fa-check-circle"></i></div>
                                    </div>
                                    ...
                                    <div class="bus-v2-trip-card-body">
                                        <div class="bus-v2-trip-main">
                                            ...
                                            <div class="bus-v2-route-operator-card {{ $m2['isBla'] ? 'is-bla' : 'is-flix' }}">
                                                <span class="bus-v2-route-operator-logo">
                                                    <img src="{{ $m2['logo'] }}" alt="{{ $m2['operator'] }}">
                                                </span>
                                                <span>{{ $m2['operator'] }}</span>
                                            </div>
                                        </div>
                                        <div class="bus-v2-trip-side">
                                            @if (!empty($bus['available_seats']))
                                            ...
                                            <div class="bus-v2-trip-action bus-v2-rt-return-action">
                                                <button
                                                    type="button"
                                                    class="bus-v2-button bus-v2-button--ghost bus-v2-rt-details-btn"
                                                    data-rt-view-details
                                                    data-rt-direction="return"
                                                    data-rt-trip="{{ $formatTrip($bus) }}">
                                                    <i class="fas fa-list-ul"></i>
                                                    Voir les détails
                                                </button>
                            ...
                            @empty
                                <div class="bus-v2-empty-copy">No return trips available with {{ $providerLabels[$provKey] }}</div>
                            @endforelse
                        </div>
...
<div class="bus-v2-trip-modal" data-rt-trip-modal hidden>
    ...
    <div class="bus-v2-trip-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="busV2TripDetailsTitle">
        ...
        <div class="bus-v2-trip-modal-head">
            ...
            <h3 id="busV2TripDetailsTitle" data-rt-modal-title>Loading details</h3>
            <p data-rt-modal-subtitle></p>
        </div>
        <div class="bus-v2-trip-modal-body" data-rt-modal-body>
            <div class="bus-v2-empty-copy">Loading stops and journey details...</div>
        </div>
    </div>
</div>
