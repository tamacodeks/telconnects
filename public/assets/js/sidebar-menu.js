...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    ...
    $(".sidebar-wrapper nav ul li a").filter(function () {
        var link = normalizeUrl($(this).attr("href"));
        if (link) {
            if (current == link) {
...
if($('.simplebar-wrapper .simplebar-content-wrapper') && $('#pageWrapper').hasClass('compact-wrapper')) {
  var $scrollWrap = $('.simplebar-wrapper .simplebar-content-wrapper');
  var $activeLink = $scrollWrap.find('a.active').first();
  if ($scrollWrap.length && $activeLink.length && $activeLink.offset()) {
    $scrollWrap.animate({
...
function togglePinnedName() {
  ...
  const pinTitle   = document.querySelector('.pin-title'); // e.g., the header text to show/hide
  const hasPinned  = !!document.querySelector('.pinned');  // any element with class "pinned"?

  if (!pinTitle) return; // element not present on this page; avoid crash

  // Add "show" when there is at least one pinned item; remove otherwise
  pinTitle.classList.toggle('show', hasPinned);
}
